...
- **Fallback이 본체:** 완벽한 소스가 없어도 에러 대신 _가진 것 중 최선_을 욱여넣는 구조가 메인 엔진.
...
입력(template.json + asset_db.json)
   │
   ▼
[RULE7 게이트] 소스 총량 미달 → Reject(SOURCE_INSUFFICIENT)   ← 유일한 거부
   │ 통과
   ▼
...
[Agent B: 1-Hit 검증, RULE12]
   ...
   └─ 반려 → (1-Hit, 재탐색 0회) → [Fallback ✅] semantic 버리고 리듬(motion)으로 강제 채움
                                    → edit_instruction(fallback_used=true), 절대 안 멈춤
※ 모든 단계 ok/fail·어떻게/무엇으로는 logs/engine.log + logs/runs.jsonl 기록 (터미널 X)
```

세 덩어리(`_allocate` 공유 / `agent_b_validate` / `assemble` 오케스트레이터)로 **코드에서 다중 에이전트 구조가 그대로 보이게** 분리함 → [engine.py](src/fallback_engine/engine.py).
...
│   ├─ engine.py                      ← Core Engine: Agent A / Agent B / Fallback / orchestrator
│   ├─ trace.py                       ← 파일 로깅(logs/engine.log, runs.jsonl)
│   └─ config.py                      ← 노브 로더
├─ tests\
...
│   └─ test_trace.py                 ← 파일 로깅 검증
├─ logs\                              ← 실행 로그/덤프 (gitignored)
└─ docs\schemas\                      ← 원본 예시 JSON(참고용)
```

_기획 문서:_ PROJECT_PLAN / schema_decisions / tech_stack_decisions / expansion_roadmap (루트).

---

## 7. 지금 통과하는 15개 테스트 = "증명된 것"

**계약([test_schemas.py](tests/test_schemas.py), 3):** template/asset_db 로드 OK · 구버전 골든(bgm/speed_ramp)은 거부됨(②③④ 강제 증명).

**오라클([test_invariants.py](tests/test_invariants.py), 4):** 오라클이 정상 출력 통과 · 슬롯순서 위반(RULE4) 적발 · 싱크플래그 불일치(RULE13) 적발 · 소스부족(RULE7) 판별. → _오라클이 진짜 작동함을 먼저 증명._

**엔진([test_engine.py](tests/test_engine.py), 6):**

- 소스 부족 → Reject (RULE7, 유일한 거부)
- 세그먼트 1개·슬롯 2개 → 재사용+수미상관 (RULE10)
- 의미 매칭: 컷 벡터에 맞는 세그먼트가 슬롯 차지 (RULE6 코사인)
- Agent B: 비트에 맞으면 통과(fallback 아님)
- off-beat → Agent B 반려 → **Fallback**(fallback_used=true, 무중단)
- 충분한 소스 → 항상 결과(절대 reject 안 함)

**로깅([test_trace.py](tests/test_trace.py), 2):** 매 실행이 파일에 덤프됨 · 로거가 터미널로 안 샘(propagate=False).
...
- **Fallback refine** — 현재 Fallback은 "어떻게든 반환"까지만. 로그가 드러낸 약점: 구조적 문제(off-beat 템플릿)는 Fallback도 못 고치고 Agent A와 **같은 picks**를 반환. 원안의 "리듬 사수"를 더 충실히 다듬을 차례(개선안 검토 중).
- 그 후: **Yellow**(추출) / **Blue**(브라우저) / **Pink**(렌더) — 전부 미착수.
...
**관찰 로그:** `logs/engine.log`(사람용 한 줄씩), `logs/runs.jsonl`(실행당 JSON 덤프). 터미널엔 안 찍힘 — fallback refine은 이 로그 근거로 진행.
...
- 그래서 포폴은 _"잘 만든 엔진 + 시장 안목"_으로 승부. 커모디티 부분(비트싱크/자막/렌더)은 "의도적으로 재발명 안 함"으로 명시.
...
| `d25cdc1` | STEP 3d: Fallback(무중단) + 파일 로깅 |
